...
private:
    ...
    MapElement***  MapPos;   //000000________0000000
public:
    ...
    {
        MapPos = new MapElement**[num_rows];
        for (int i = 0; i < num_rows; i++)
        {
            MapPos[i] = new MapElement*[num_cols];
        }
        
      /*  for (int i = 0; i < num_walls; ++i) {
            int row = array_walls[i].row;
            int col = array_walls[i].col;
            map[row][col] = new Wall();
        }

         for (int i = 0; i < num_fake_walls; ++i) {
            int row = array_fake_walls[i].row;
            int col = array_fake_walls[i].col;
    ...
    bool isValid ( const Position & pos , MovingObject * mv_obj ) const ; // <0>_<0>
};
